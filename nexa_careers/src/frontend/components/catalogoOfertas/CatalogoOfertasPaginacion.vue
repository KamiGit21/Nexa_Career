<template>
  <div v-if="totalPaginas > 1" class="flex items-center justify-center gap-1.5 mt-8 mb-0 select-none">
    ...
    <button @click="$emit('cambiar', paginaActual - 1)" :disabled="paginaActual === 1"
    ...
    <template v-if="paginasVisibles[0] > 1">
      <button @click="$emit('cambiar', 1)" class="w-10 h-10 rounded-xl text-sm font-semibold transition-all duration-200
               border border-gray-200 bg-white text-gray-600
               hover:border-[#1b2a4a] hover:text-[#1b2a4a] hover:shadow-sm">1</button>
      <span v-if="paginasVisibles[0] > 2" class="text-gray-400 text-sm px-1">···</span>
    </template>

    <button v-for="pagina in paginasVisibles" :key="pagina" @click="$emit('cambiar', pagina)"
      class="w-10 h-10 rounded-xl text-sm font-semibold transition-all duration-200" :class="pagina === paginaActual
    ...
    <template v-if="paginasVisibles[paginasVisibles.length - 1] < totalPaginas">
      <span v-if="paginasVisibles[paginasVisibles.length - 1] < totalPaginas - 1"
        class="text-gray-400 text-sm px-1">···</span>
      <button @click="$emit('cambiar', totalPaginas)" class="w-10 h-10 rounded-xl text-sm font-semibold transition-all duration-200
               border border-gray-200 bg-white text-gray-600
               hover:border-[#1b2a4a] hover:text-[#1b2a4a] hover:shadow-sm">{{ totalPaginas }}</button>
    </template>

    <button @click="$emit('cambiar', paginaActual + 1)" :disabled="paginaActual === totalPaginas"
      class="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200
             border border-gray-200 bg-white text-gray-600
             hover:border-[#1b2a4a] hover:text-[#1b2a4a] hover:shadow-sm
             disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:border-gray-200 disabled:hover:text-gray-600 disabled:hover:shadow-none">
