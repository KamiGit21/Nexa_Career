  ...
  <header class="bg-[#1b2a4a] text-white py-16 px-8 text-center">
    ...
    <div class="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-4">
      ...
      <div class="flex-1 w-full">
        <div
          ...
          <div class="flex-1 flex items-center px-4 gap-3 border-b md:border-b-0 md:border-r border-gray-100">
            ...
            <input
              ...
              @input="updateFiltro('titulo', $event.target.value)"
          ...
          <div class="flex-1 flex items-center px-4 gap-3 border-b md:border-b-0 md:border-r border-gray-100">
            ...
            <input
              ...
              @input="updateFiltro('empleador', $event.target.value)"
              @keyup.enter="$emit('buscar')"
              type="text"
              placeholder="Empresa..."
              class="w-full py-3 text-slate-800 outline-none text-sm font-medium placeholder:text-gray-400 bg-transparent"
          ...
          <div class="flex-[0.7] flex items-center px-4 gap-3">
            ...
            <select
              ...
              @change="updateFiltro('modalidad', $event.target.value)"
        ...
        <!-- Fila secundaria: filtro por fecha -->
        <div class="mt-3 flex flex-wrap items-center justify-between gap-3">
          <FiltroPorFecha
            :model-value="filtros.rangoFecha"
            @update:model-value="val => { updateFiltro('rangoFecha', val); $emit('buscar') }"
          />
          <span
  v-if="filtros.rangoFecha.preset || filtros.rangoFecha.desde || filtros.rangoFecha.hasta"
  class="text-xs text-[#d0b06d] cursor-pointer hover:underline"
  @click="updateFiltro('rangoFecha', { preset: '', desde: '', hasta: '' }); $emit('buscar')"
>
...
const props = defineProps({
  filtros: {
    type: Object,
    default: () => ({ titulo: '', empleador: '', modalidad: '', rangoFecha: { preset: '', desde: '', hasta: '' } })
  },
