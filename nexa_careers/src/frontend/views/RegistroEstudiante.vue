<template>
  <div class="bg-[#fffffd] w-full min-w-[1440px] min-h-[1926px] relative">
    ...
    <nav class="absolute top-0 left-0 w-full h-[126px] flex items-center">
      <div class="absolute top-12 left-[952px] text-[#d0b06d] text-[25px] cursor-pointer whitespace-nowrap">Home</div>
      <div class="absolute top-12 left-[1089px] text-[#d0b06d] text-[25px] cursor-pointer whitespace-nowrap">Registrarse</div>
    ...
    <div class="absolute top-[205px] left-[150px] w-[1139px] bg-white rounded-[25px] pb-20">
      ...
      <form @submit.prevent="handleSubmit" class="px-[91px] mt-8" novalidate>
        <div class="flex gap-8 mb-6">
          <div class="flex flex-col gap-3 w-1/2">
            ...
            <input id="nombre" v-model="form.nombre" type="text" autocomplete="given-name"
              class="w-full h-[50px] rounded-[15px] border border-[#002349] px-4 text-[#002349] text-xl focus:outline-none focus:ring-2 focus:ring-[#002349]" />
          </div>
          <div class="flex flex-col gap-3 w-1/2">
        ...
        <div class="flex items-center gap-3 mb-3">
          <div class="flex-1 h-2.5 bg-[#d9d9d9] rounded-full overflow-hidden">
            <div class="h-full rounded-full transition-all duration-300"
              :style="{ width: porcentajeFuerza + '%', backgroundColor: colorFuerza }" />
          </div>
          <span class="text-[#545454] text-lg whitespace-nowrap">{{ etiquetaFuerza }}</span>
        </div>

        <ul class="mb-6 space-y-1 list-disc list-inside text-xl">
        ...
        <div class="flex justify-center">
          <router-link to="/registro-empleador" class="w-[474px] h-[65px] bg-[#002349] rounded-[15px] text-[#d0b06d] text-[35px] font-semibold hover:bg-[#003366] transition-colors flex items-center justify-center">
    ...
    <div v-if="showModal" class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[770px] bg-neutral-100 rounded-[20px] z-50 p-10 text-center shadow-2xl">
      <p class="text-[#06b500] text-[40px] font-semibold mb-10">¡El usuario fue creado exitosamente!</p>
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      // TODO: POST /api/estudiantes  →  { nombre, apellido, gmail: correo, telefono, contrasena }
      this.showModal = true
    }
  }
}
