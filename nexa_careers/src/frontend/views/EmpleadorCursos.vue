      ...
      <template v-else>
        <div v-if="cursosFiltrados.length === 0" class="text-center py-20 text-gray-400">
          ...
          <p class="text-lg font-medium">
            {{ (categoriaFiltro || estadoFiltro !== null) ? 'No hay cursos con los filtros aplicados' : 'Aún no has publicado ningún curso' }}
          </p>
          <router-link v-if="!categoriaFiltro && estadoFiltro === null" to="/publicar-curso"
...
import {
  listarCursosPorEmpleador,   // ← diferencia 1
  listarCategorias,
  archivarCurso,
  desarchivarCurso
...
const cargarCursos = async () => {
  ...
  try {
    ...
    if (!sesion.id || sesion.rol !== 'empleador') { router.push('/login'); return }  // ← diferencia 2
    const res = await listarCursosPorEmpleador(sesion.id)
