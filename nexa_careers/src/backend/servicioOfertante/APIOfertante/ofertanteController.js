...
export const crearOfertante = async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO postulante.postulante (id_oferta, id_estudiante, estado) VALUES (?, ?, ?)`,
...
export const buscarOfertantePorId = async (req, res) => {
  const { id } = req.params;
  try {
    const [rows] = await db.query('SELECT * FROM postulante.postulante WHERE id_postulante = ?', [id]);
...
export const buscarPorOferta = async (req, res) => {
  ...
  try {
    const [rows] = await db.query('SELECT * FROM postulante.postulante WHERE id_oferta = ?', [id_oferta]);
...
export const buscarPorEstudiante = async (req, res) => {
  ...
  try {
    const [rows] = await db.query('SELECT * FROM postulante.postulante WHERE id_estudiante = ?', [id_estudiante]);
...
export const obtenerPostulacionesEstudiante = async (req, res) => {
  ...
  try {
    const query = `
      ...
      INNER JOIN oferta.oferta o ON p.id_oferta = o.id_oferta
...
export const cambiarEstadoOfertante = async (req, res) => {
  ...
  try {
    const [result] = await db.query('UPDATE postulante.postulante SET estado = ? WHERE id_postulante = ?', [estado, id]);
  ...
  try {
    ...
    for (const postulante of rows) {
      const [estudianteRows] = await db.query(
        'SELECT id_estudiante, nombre, apellido, gmail, telefono, cv FROM estudiante.estudiante WHERE id_estudiante = ?',
...
export const contarOfertantesEmpleador = async (req, res) => {
  const { empleador } = req.params;
  if (empleador === undefined) {
    return res.status(400).json({ 
      success: false, 
      message: 'Debes proporcionar un ID de empleador para contar sus postulaciones.' 
    });
  }
  ...
  try {
   const [result] = await db.query(
      `SELECT COUNT(*) as total 
       ...
       WHERE o.id_empleador = ? AND p.estado = 1`,
      [empleador]
    );
    res.status(200).json({
      success: true,
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Error interno al intentar contar las postulaciones' 
    });
  }
};